...
# Prompt to enter a password for the PostgreSQL movienite user (rather than hard-coding
# a password in this script).
read -p "Enter password for movienite DB user: " DB_PASSWORD
...
# Set up the movienite DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE movienite"
sudo -i -u postgres psql -d movienite -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d movienite -c "CREATE ROLE movienite WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the movienite database to the system-wide environment
# variables in the /etc/environment file.
echo "MOVIENITE_DB_DSN='postgres://movienite:${DB_PASSWORD}@localhost/movienite'" >> /etc/environment
